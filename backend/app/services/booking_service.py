...
class BookingService:
    ...
    def create_booking(self, booking_data: BookingCreate, borrower_id: str) -> Booking:
        ...
        booking_point = self.db.query(BookingPoint).filter(
            BookingPoint.id == booking_data.booking_point_id
        ).first()
        if not booking_point:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND,
                detail="Пункт выдачи не найден"
            )
        ...
        existing_booking = self.db.query(Booking).filter(
            and_(
                ...
                Booking.status.in_([BookingStatus.PENDING, BookingStatus.CONFIRMED, BookingStatus.TAKEN])
            )
        ).first()
    ...
    def get_user_bookings(self, user_id: str, search_params: BookingSearchParams) -> Tuple[List[Booking], int]:
    ...
    def update_booking_status(self, booking_id: str, new_status: BookingStatus, user_id: str) -> Optional[Booking]:
        ...
        if book.owner_id == user_id:
            ...
            elif new_status == BookingStatus.CANCELLED and old_status in [BookingStatus.PENDING, BookingStatus.CONFIRMED]:
                booking.status = new_status
        ...
        # Заемщик может отменить только pending бронирование
            ...
            if new_status == BookingStatus.CANCELLED and old_status == BookingStatus.PENDING:
                booking.status = new_status
                # Возвращаем доступность книги
                book.is_available = True
                book.updated_at = datetime.utcnow()
