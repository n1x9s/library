...
class NotificationService:
    ...
    def get_user_notifications(self, user_id: str, limit: int = 50, offset: int = 0) -> List[Notification]:
    ...
    def create_booking_notification(self, booking: Booking) -> Notification:
        ...
        book = self.db.query(Booking.book).join(Booking.book).filter(Booking.id == booking.id).first()
        if not book:
            return None
    ...
    def create_return_reminder(self, booking: Booking) -> Notification:
        ...
        message = f"Не забудьте вернуть книгу '{book.title}' до {booking.planned_return_date}"
    ...
    def create_book_available_notification(self, book_id: str, user_id: str) -> Notification:
